...
def get_lattice_spacings(material):
    ...
    if material.lower() == "aupd":
        hkls = {"111": 2.31, "200": 2.00, "220": 1.41, "311": 1.21, "222": 1.15}
        return hkls
    elif material.lower() == "au":
        hkls = {"111": 2.36, "200": 2.04, "220": 1.44, "311": 1.23, "222": 1.18}
        return hkls
    elif material.lower() == "si":
        hkls = {"111": 3.14, "200": 2.72, "220": 1.92, "311": 1.64, "222": 1.57}
        return hkls
...
calibration_file_80kV = os.path.join(
    merlintools_path, "calibrations", "80kV_calibrations.json"
)
with open(calibration_file_80kV, "r") as fp:
    cal_80kV = json.load(fp)

calibration_file_200kV = os.path.join(
    merlintools_path, "calibrations", "200kV_calibrations.json"
)
with open(calibration_file_200kV, "r") as fp:
    cal_200kV = json.load(fp)

calibration_file_300kV = os.path.join(
    merlintools_path, "calibrations", "300kV_calibrations.json"
)
...
def get_calibration(beam_energy, cl, units="mrads"):
    ...
    if beam_energy == 80:
        ...
        raise ValueError(
            "No calibration for beam energy: %s. "
    ...
    if str(cl) in calibration_dictionary.keys():
        ...
        calibration = np.round(_extrapolate_calibration(cl, calibration_dictionary), 3)
        logger.info(
            "Camera length not in calibration table. Calibration will be extrapolated."
        )
    ...
    if units == "mrads":
        ...
        raise ValueError(
            "Units (%s) not understood. Must be 'mrads', 'q', or 'k'" % units
        )
    return calibration
...
def load_calibration_file(beam_energy, camera_length, also_alpha_data=False):
    datapath = os.path.join(
        merlintools_path, "calibrations", "data", "%skV" % beam_energy
    )
...
def get_2d_scattering_profile(
    Z, composition, q_range=[0, 2], q_size=256, plot_result=False, figsize=(9, 4)
):
    """
...
def get_simulated_data():
    datafile = os.path.join(
        merlintools_path, "tests", "test_data", "SimulatedData.hdf5"
    )
...
def calc_euler(uvw):
    """
    Calculate crystal direction rotation.

    Calculates the Euler rotation angles in ZXZ notation need to to align a given crystal direction [uvw]
    with the incident beam direction [001].

    Parameters
        uvw : list or numpy.ndarray
            Miller indices for the crystal direction to be rotated

    Returns
        euler_zxz : numpy.ndarray
            Euler angles for rotation
    """
    beam_direction = np.array([0, 0, 1])
    uvw = uvw / np.linalg.norm(uvw)
    rotation = Rotation.align_vectors(beam_direction[np.newaxis, :], uvw[np.newaxis, :])
    euler_zxz = rotation[0].as_euler("zxz")
    euler_zxz = np.degrees(euler_zxz)
    return euler_zxz
