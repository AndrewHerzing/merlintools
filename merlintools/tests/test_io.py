...
class TestMIBConversion:
    ...
    def test_dm_scan_missing_frames(self):
        datadir = os.path.join(
            merlin_path, "tests", "test_data", "DM-Scan_MissingFrames"
        )
        mib = glob.glob(datadir + "/*.mib")
        hdr = glob.glob(datadir + "/*.hdr")
        emi = glob.glob(datadir + "/*.emi")
...
class TestGetMerlinParameters:
    def test_get_merlin_parameters_string(self):
        h5file = os.path.join(merlin_path, "tests", "test_data", "FPD_HDF5.hdf5")
...
class TestHeaderParsing:
    ...
    def test_hdr_parser(self):
        ...
        hdrfile = os.path.join(
            merlin_path, "tests", "test_data", "TestData_12bit", "merlin.hdr"
        )
    ...
    def test_mib_parser(self):
        """Parse MIB file."""
        mibfile = os.path.join(
            merlin_path, "tests", "test_data", "TestData_12bit", "merlin.mib"
        )
...
class TestReadSingleMIB:
    def test_load_single_mib(self):
        mibfile = os.path.join(merlin_path, "tests", "test_data", "SingleMIB.mib")
