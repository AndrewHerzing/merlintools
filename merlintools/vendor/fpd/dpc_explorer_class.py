...
class DPC_Explorer:
    def __init__(
        ...
        cmap_nfold : int
            The n-fold rotational symmetry of the colour map.
        median : bool
        ...
        if isinstance(d, int):
            dim = np.indices((np.abs(d),) * 2, dtype=float)
        ...
        if isinstance(pct, (int, float)):
            self._pct = ((pct,) * 2,) * 2
        elif isinstance(pct, (Sequence, np.ndarray)):
            y_pct = pct[0]
            x_pct = pct[1]
        ...
        # CMAP
        self._init_cmap(cmap, cmap_nfold)
        self._cmap_nfold = cmap_nfold
    ...
    def _make_scan_indices(self):
        self._inds = np.indices(self._y_orig.shape, dtype=float)
        self._inds -= self._inds.mean((1, 2))[:, None, None]  # centered

    def _init_cmap(self, cmap_in, cmap_nfold):
        self._cmaps = OrderedDict()  # dict of all cmaps
        ...
        clr_interp = [
            np.interp(theta, thetap, clr_array[:, i], period=360) for i in range(3)
        ]
        ...
        # set nfold
        if cmap_nfold != 1:
            for cmap_ki, cmap_vi in self._cmaps.items():
                n = max(512, cmap_vi.N * cmap_nfold)
                cmap_vi = cmap_vi(np.linspace(0, 1, n))
                self._cmaps[cmap_ki] = mpl.colors.ListedColormap(
                    np.vstack((cmap_vi,) * cmap_nfold)
                )
    ...
    def _calc_data(self, calc_ransac=False):
        ...
        # descan
        self._descany = (
            self._inds[0] * self._descanY_factY + self._inds[1] * self._descanY_factX
        )
        self._descanx = (
            self._inds[0] * self._descanX_factY + self._inds[1] * self._descanX_factX
        )
        ...
        # percentile
        if self._pct:
            ymin, ymax = np.percentile(self.y, [self._pct[0][0], 100 - self._pct[0][1]])
            self.y = self.y.clip(ymin, ymax)
            xmin, xmax = np.percentile(self.x, [self._pct[1][0], 100 - self._pct[1][1]])
    ...
    def _plot_xy(self):
        ...
        f, (ax1, ax2) = plt.subplots(
            1, 2, sharex=True, sharey=True, figsize=(6.4, 4), subplot_kw=kwd
        )
        self._figs.append(f)
        ...
        for ax, d, t in zip((ax1, ax2), [y, x], ["y", "x"]):
            im = ax.imshow(
                d, interpolation="nearest", cmap="gray", vmin=vmin, vmax=vmax
            )
        ...
        self._xy_selector = RectangleSelector(
            ax1,
    ...
    def _hist_tit_str(self):
        tit = (
            "(cy, cx), (r_min, r_max): (%0.3f, %0.3f), (%0.3f, %0.3f)    dt: %0.3f\nDescan Y(y,x), X(y,x) x1000: (%0.3f, %0.3f), (%0.3f, %0.3f)\n"
            % (
                self._yc,
                self._xc,
                self._r_min,
                self._r_max,
                self._dt / np.pi * 180,
                self._descanY_factY * 1000,
                self._descanY_factX * 1000,
                self._descanX_factY * 1000,
                self._descanX_factX * 1000,
            )
        )
        return tit
    ...
    def _plot_hist(self):
        ...
        if self._hist_lims is None:
            ...
            ymin, ymax = np.percentile(ys, [self._pct[0][0], 100 - self._pct[0][1]])
            xmin, xmax = np.percentile(xs, [self._pct[1][0], 100 - self._pct[1][1]])
        ...
        yc = (ymin + ymax) / 2.0
        xy_bins = [
            np.linspace(xc - rng, xc + rng, self._nbins + 1),
            np.linspace(yc - rng, yc + rng, self._nbins + 1),
        ]

        H, self._xedges, self._yedges = np.histogram2d(
            self.x.flatten(), self.y.flatten(), bins=xy_bins
        )
        ...
        cmap = copy.copy(cmap)
        cmap.set_bad("w")
        self._hist_im = ax.pcolormesh(
            self._xedges, self._yedges, self._Hmasked, cmap=cmap
        )

        self._circ_r_max = mpl.patches.Circle(
            (self._xc, self._yc), self._r_max, ec="r", fc="none", ls="solid", lw=2
        )
        self._circ_r_min = mpl.patches.Circle(
            (self._xc, self._yc), self._r_min, ec="r", fc="none", ls="solid", lw=1.25
        )
        ...
        self._rect = mpl.patches.Rectangle(
            (self._xc - rwh / 2, self._yc - rwh / 2), rwh, rwh, ec="k", fc="none", lw=2
        )
        ...
        self._sgaus = Slider(
            self._axgaus, "Gaus.", 0.0, self._gaus_lim, valinit=self._gaus
        )
        self._svectrot = Slider(
            self._axvectrot, "Rot.", 0.0, 360.0, valinit=self._vectrot
        )
        ...
        self._radiom = RadioButtons(
            raxm, ["med.", "med. of dif."], active=self._median_mode
        )
    ...
    def get_image_sequence(self, im_types="tr_im", images=False):
        ...
        if images:
            ...
            for i, imt in enumerate(im_types):
                if imt in raw:
                    # autoscale
                    mins[i], maxs[i] = np.percentile(
                        ims[i], [0, 100], axis=(-3, -2, -1)
                    )
                elif imt in zt2pi:
                    maxs[i] = 2 * np.pi
                elif imt in zto:
    ...
    def _update_hist_calc(self):
        ...
        H, _, _ = np.histogram2d(
            x.flatten(), y.flatten(), bins=[self._xedges, self._yedges]
        )
        # H needs to be rotated and flipped
        H = H.T
        # Mask pixels with a value of zero
    ...
    def _plot_rt(self):
        ...
        f, axs = plt.subplots(
            1, 3, sharex=True, sharey=True, subplot_kw=kwd, figsize=(12, 4.4)
        )
        self._figs.append(f)
        f.canvas.set_window_title("rt")
        ims = []
        for i, (ax, d, t) in enumerate(
            zip(axs.flat, [self.rn] + self._t_ims, ["radius", "theta", "theta (r)"])
        ):
            if i == 0:
    ...
    def _plot_cw(self):
        ...
        try:
            ...
            f, axs = plt.subplots(
                1, 2, subplot_kw=dict(projection="polar"), figsize=(6.4, 3)
            )
            self._cw_fig = f
            self._figs.append(f)
        ...
        for ax, pd in zip(axs, [A_rgb, A_rgb_rv]):
            ...
            ax.clear()
            im = ax.pcolormesh(
                T, R, values, color=pd.reshape((-1, 3)), shading="gouraud"
            )
            im.set_array(None)
            ax.set_axis_off()
    ...
    def _on_save(self, event, post_tag=None, working_dir=None):
        ...
        for fn, imob in zip(["rn", "tc", "tcrv"], self._im_objs):
            ...
            mpl.image.imsave(
                os.path.join(path, now + "_" + fn + ".png"), im_data, cmap="gray"
            )
        ...
        for fn, imob in zip(["y", "x"], self._xy_ims):
            ...
            mpl.image.imsave(
                os.path.join(path, now + "_" + fn + ".png"),
                im_data,
                cmap="gray",
                vmin=vmin,
                vmax=vmax,
        ...
        for fn, arr in zip(["y_orig", "x_orig"], [self._y_orig, self._x_orig]):
            ...
            mpl.image.imsave(
                os.path.join(path, now + "_" + fn + ".png"), arr, cmap="gray"
            )

        # (r, theta)
        np.savetxt(os.path.join(path, now + "_" + "t" + ".dat.gz"), self._t)
        mpl.image.imsave(
            os.path.join(path, now + "_" + "r" + ".png"), self.r, cmap="gray"
        )
        ...
        for fn, ax in zip(["flat", "val"], f.axes):
            ...
            f.savefig(
                os.path.join(path, now + "_cw_" + fn + ".png"),
                bbox_inches=extent,
                dpi=300,
            )
        print("Done")
    ...
    def _on_print(self, event):
        ...
        cmd_str.append("r_min={:0.4f}".format(self._r_min))
        cmd_str.append("r_max={:0.4f}".format(self._r_max))
        cmd_str.append(
            "descan=[{:0.4f}, {:0.4f}, {:0.4f}, {:0.4f}]".format(
                self._descanY_factY * 1000,
                self._descanY_factX * 1000,
                self._descanX_factY * 1000,
                self._descanX_factX * 1000,
        ...
        cmd_str.append("cyx=[{:0.4f}, {:0.4f}]".format(self._yc, self._xc))
        cmd_str.append("vectrot={:0.2f}".format(self._vectrot))
        cmd_str.append("gaus={:0.2f}".format(self._gaus))
        cmd_str.append(
            "pct=[[{:0.3f}, {:0.3f}], [{:0.3f}, {:0.3f}]]".format(
                self._pct[0][0], self._pct[0][1], self._pct[1][0], self._pct[1][1]
            )
        )
        cmd_str.append("dt={:0.2f}".format(self._dt))
        cmd_str.append(
            "cmap={:d}".format(list(self._cmaps.keys()).index(self._cmap_curent[0]))
        )
        cmd_str.append("cmap_nfold={:d}".format(self._cmap_nfold))
        cmd_str.append("median={:s}".format(str(self._median)))
        cmd_str.append("median_mode={:d}".format(self._median_mode))
        cmd_str.append("ransac={:s}".format(str(self._ransac)))
        cmd_str.append("ransac_dict={:s}".format(str(self._ransac_dict)))
        cmd_str.append("flip_y={:s}".format(str(self._flip_y)))
        cmd_str.append("flip_x={:s}".format(str(self._flip_x)))
        cmd_str.append("origin={:s}".format(str(self._origin)))
        cmd_str.append("yx_range_from_r={:s}".format(str(self._yx_range_from_r)))
    ...
    def _on_motion(self, event):
        ...
        if self._press_cnt:
            ...
            # must be in circle
            # print('x0=%f, xpress=%f, event.xdata=%f, dx=%f, x0+dx=%f'%(x0, xpress, event.xdata, dx, x0+dx))
            r0 = np.hypot(xpress - x0, ypress - y0)
        ...
        for cmap in self._cmaps.values():
            ...
            for i, c in enumerate(clrs):
                rect = patches.Rectangle(
                    (i, 0), 1, 3, linewidth=0, edgecolor=None, facecolor=c
                )
                ax.add_patch(rect)
