...
def _parse_dm_tia_file(fn):
    ...
    if 'dm' not in ext.lower():
        ...
        if valid_n_ims > 1:
            _logger.info("Only the first of %d valid TIA image data in '%s' will be stored as an EMD." %(valid_n_ims, fn))
...
def _create_dm_tia_grp(fgp, gname, im_class, dim_names, dim_axes, dim_units,
               dmtia_fn, dmtia_tagd, dmtia_bin, *args, **kwargs):
    '''
...
def _write_dm_files_to_hdf5(group, base_kwd, dmfns,
    ...
    for i, dmfn in enumerate(dmfns):
        ...
        _create_dm_tia_grp(group, 'DM%d' %(i), True, 
                           dim_names[:2], dim_axes[:2], dim_units[:2],
                           dmfn, tagd, dm_bin, **kwd)


def _write_tia_files_to_hdf5(group, base_kwd, tiafns,
                             dim_lens, dim_names, dim_axes, dim_units):
    '''
    Write TIA file(s)
    '''
    ...
    for i, tiafn in enumerate(tiafns):
        ...
        kwd.update({'data' : tia_im,
                    'dtype' : TIA_dtype}) 

        _create_dm_tia_grp(group, 'TIA%d' %(i), True, 
                           dim_names[:2], dim_axes[:2], dim_units[:2],
                           tiafn, tagd, tia_bin, **kwd)
...
class MerlinBinary:
    def __init__(self, binfns, hdrfn, dmfns=[], tiafns=[], ds_start_skip=0,
                 row_end_skip=1, scanYalu=None, scanXalu=None, detYalu=None,
                 detXalu=None, detY=256, detX=256, sort_binary_file_list=True, 
    ...
    def _determine_axes(self):
        ...
        if len(self._binfns) == 1 and self._scanXalu is None and self._scanYalu is None and len(self._dmfns) == 0 and len(self._tiafns) == 0:
            ...
            if len(self._dmfns) > 0:
                ...
                _logger.exception("Either provide DM or TIA files or specify the scan parameters through `scanYalu` and `scanXalu`.")
    ...
    def write_hdf5(self, h5fn=None, chunks=None, scan_chunks=(16, 16), im_chunks=(16, 16),
        ...
        with h5py.File(h5fn, 'w', driver=None) as h5f:
            ...
            # Write TIA files
            _write_tia_files_to_hdf5(fpd_expt_grp, emd_ds_default, self._tiafns,
                                    self._dim_lens, self._dim_names, self._dim_axes, self._dim_units)
...
def _dm_tia_tags_to_dict(fpg, fpd_check=True, group_attr='dm_group_type', group_attr_val=1, group_fn='dm_filename'):
    '''
...
def hdf5_dm_tags_to_dict(fpg, fpd_check=True):
    ...
    rtn = _dm_tia_tags_to_dict(fpg, fpd_check,
...
def _dm_tia_to_bin(fpg, fns=None, fpd_check=True, ow=False,
                   group_attr='dm_group_type', group_attr_val=1, group_fn='dm_filename'):
    '''
    ...
    group_paths = find_hdf5_objname_by_attribute(h5f, group_attr,
                                                 group_attr_val, False)
    n_files = len(group_paths)
    print('Found %d file(s).' %(n_files))
    ...
    for fn, grp in fn_gn:
        ...
        if not ow and os.path.isfile(fn):
            reply = input("File '%s' already exists. Overwrite? ([y]/n): " %(os.path.abspath(fn)) )
...
def hdf5_dm_to_bin(fpg, dmfns=None, fpd_check=True, ow=False):
    ...
    _dm_tia_to_bin(fpg, dmfns, fpd_check, ow,
                   group_attr='dm_group_type', group_attr_val=1, group_fn='dm_filename')
...
def hdf5_tia_to_bin(fpg, tiafns=None, fpd_check=True, ow=False):
    ...
    _dm_tia_to_bin(fpg, tiafns, fpd_check, ow,
                   group_attr='tia_group_type', group_attr_val=1, group_fn='tia_filename')
