...
def disk_image(
    ...
    # blur
    if sigma != 0:
        disk = sp.ndimage.filters.gaussian_filter(
            disk,
            sigma=sigma * upscale,
            order=0,
            mode="constant",
    ...
    if upscale != 1:
        if ds_method == "rebin":
            ...
            ns = np.array(
                [np.ceil(t / float(upscale)) * upscale for t in disk.shape], dtype=int
            )
            ...
            disk = np.pad(disk, pad_width=pw, mode="constant", constant_values=0)
            disk = rebinA(
                disk, int(disk.shape[0] / upscale), int(disk.shape[1] / upscale)
            ) / float(upscale**2)
        if ds_method == "interp":
            # Bi-cubic down scale
            kwd = {
...
def shift_array(scan_len=32, shift_min=-8.0, shift_max=8.0, shift_type=0):
    ...
    if shift_type == 0:
        ...
        shiftyy = (
            np.random.random_sample(scan_im_shape) * (shift_max - shift_min)
...
def shift_images(
    ...
    partial_f = partial(
        shift_im, image, noise=noise, method=method, fill_value=fill_value
    )
    ...
    elif np.issubdtype(target_dtype, float):
...
# --------------------------------------------------
def segmented_detectors(
    im_shape=(256,) * 2, rio=(28, 64), cyx=None, ac_det_roll=0, dtype="u2"
):
    """
