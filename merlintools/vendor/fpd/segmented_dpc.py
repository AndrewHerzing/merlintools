...
from .fpd_processing import rebinA, rotate_vector
from .utils import nearest_int_factor
...
def load_gla_dpc(ds, path=None, ext="dm3"):
    ...
    fns = ["KE INT " + str(t) for t in range(4)] + [
        "KE EXT " + str(t) for t in range(4)
    ]
    fn_optional = ["Gatan HAADF", "JEOL NCB", "Noise image"]
        ...
        try:
            ...
            if fni in fns:
                ...
                with warnings.catch_warnings():
                    ...
                    if offset == 0.0:
                        warnings.warn(
                            "%s brightness origin seems to be zero. Were the trims set?"
                            % (im_name),
                            UserWarning,
                        )
                t.data -= offset
    ...
    if opt_dict["Noise image"] is not None:
        opt_dict["Noise image"].axes_manager[0].scale = (
            ds_dict[fns[0]].axes_manager[0].scale
        )
        opt_dict["Noise image"].axes_manager[1].scale = (
            ds_dict[fns[0]].axes_manager[1].scale
        )
...
class SegmentedDPC:
    def __init__(
        ...
        if sum_method.lower() not in sum_method:
            msg = "'sum_method' '%s' not known; known methods: " % sum_method + str(
                self.sum_methods
            )
            raise ValueError(msg)
        ...
        self.dpc = 1.0 - 4.0 / np.pi * (
            np.arctan(np.sqrt((1.0 - bar) / (1.0 + bar)))
    ...
    def _beta_from_dpc_sig(self, dpc_sig):
        if self.method == "accurate":
            ...
            return (
                np.sign(dpc_sig)
                * np.interp(np.abs(dpc_sig), self.dpc, self.bar)
                * self.alpha
            )
    ...
    def _correct_for_scan_angle(self):
        ...
        self.dpc_betay, self.dpc_betax = rotate_vector(
            [self.dpc_betay, self.dpc_betax], self.scan_deg
        )
        if self._oct_seg:
            self.mdpc_betay, self.mdpc_betax = rotate_vector(
                [self.mdpc_betay, self.mdpc_betax], self.scan_deg
            )
