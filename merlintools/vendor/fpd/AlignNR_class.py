...
class AlignNR:
    ...
    def plot(self):
        ...
        tit = ax1.set_title(
            "i: %02d, nrmse: %0.4f, dyx_max: %0.4f" % (self.i, self.nrmse, 0)
        )
        ...
        def update_plot(val):
            ...
            s = np.ceil(np.prod(shape) ** 0.5 / 24).astype(int)
            s = max(s, 1)
            s = np.s_[::s, ::s]
            try:
                self._plt_q.remove()
            ...
            tit.set_text(
                "i: %02d, nrmse: %0.4f, dyx_max: %0.4f"
                % (self.i, self.nrmse, dyxr.max())
            )
            ...
            # error etc
            for axi, pdi, ci in zip(
                [ax2, ax3, ax4], [ax2_pd, ax3_pd, ax4_pd], ["b-", "g-", "r-"]
            ):
                try:
        ...
        with warnings.catch_warnings():
            if self._stack_len == 1:
                warnings.simplefilter("ignore")
            sslice = Slider(
                axslice, "Slice", 0, self._stack_len - 1, valinit=0, valstep=1
            )
        ...
        salpha = Slider(
            axalpha, "alpha", 0.1, max(self.alpha * 4, 50), valinit=self.alpha
        )
        ...
        sregsig = Slider(
            ...
            "reg_sigma",
            0.1,
            max(self.reg_sigma * 4, 50),
            valinit=self.reg_sigma,
        )
    ...
    def _condition_params(self):
        ...
        if ref_is_int:
            ...
            if self._reference.lower() not in self._ref_modes:
                raise Exception(
                    "'reference' (%s) must be in: " % (self._reference), self._ref_modes
                )
        ...
        if callable(self._reg_mode):
            ...
            if callable(self._reg_mode[0]):
                ...
                # string
                raise Exception(
                    "'reg_mode' (%s) must be in: " % (self._reg_mode), self._reg_modes
                )

        # broadcast dyx0 if needed
        if self._dyx0 is not None:
            if self._dyx0.ndim == 3:
                self._dyx0 = np.repeat(
                    self._dyx0[None], len(self._images_normed), axis=0
                )

        # check nrmse_mode
        if self._nrmse_mode.lower() not in self._nrmse_modes:
            raise Exception(
                "'nrmse_mode' (%s) must be in: " % (self._nrmse_mode), self._nrmse_modes
            )
        ...
        if self._nthreads is not None and self._nthreads >= nt - 1:
            warnings.warn(
                "'nthreads' (%d) >= threads - 1 (%d); performance may be reduced."
                % (self._nthreads, nt - 1)
            )
        ...
        if self._ishift:
            if self._pct is not None:
                warnings.warn(
                    "With 'ishift=True', 'pct=None' allows intensities to be transferred correctly"
                )
    ...
    def warp(self, images, dyx=None, cval=np.nan, ishift=None):
        ...
        for j in range(len(dyx)):
            ...
            ncx = cx + dyxj[1]
            imj = map_coordinates(
                imj, np.vstack([ncy.flatten(), ncx.flatten()]), cval=cval
            )
    ...
    def _align(self):
        ...
        while not stop:
            ...
            def process_image(j):
                ...
                dyxj = -(self.images_aligned[j] - self._ref_im)[None] * (
                    self._grad_images[j] + self._grad_ref_im
                )
                ...
                if callable(self._reg_mode[0]):
                    dyxj = np.array(
                        [
                            fi(dyxji, **self._reg_kwargs)
                            for fi, dyxji in zip(self._reg_mode, dyxj)
                        ]
                    )
            ...
            with self._data_lock:
                ...
                self._dyx_metric_ax = np.append(
                    self._dyx_metric_ax, dyx_metric, axis=-1
                )

            self.criteria_bool = (
                (self.i >= self._niter),
                (self.nrmse < self._nrmse_min),
                (np.abs(self.dnrmse) < self._nrmse_rel),
            )
            ...
            if self._print_stats:
                if self.i == 1:
                    print(
                        "%5s, %9s, %9s, %s"
                        % (tuple("i nrmse dnrmse criteria_bool".split()))
                    )
                    print("------------------------------------------------------")
                print(
                    "%05d, %9.9f, %9.9f, (%r, %r, %r)"
                    % ((self.i, self.nrmse, self.dnrmse) + self.criteria_bool)
                )
