...
from merlintools.vendor import fpd
...
class TestFPDP_disc_edge(unittest.TestCase):
    def test_disc_edge_properties(self):
        ...
        im = fpd.synthetic_data.disk_image(
            intensity=1024, radius=32, sigma=sigma, size=256, noise=False
        )
        cyx, r = fpd.fpd_processing.find_circ_centre(
            im, 2, (22, int(256 / 2.0), 1), spf=1, plot=False
        )
        ...
        dp = fpdp.disc_edge_properties(
            im, sigma=6, cyx=cyx, r=r, n_angles=91, plot=False
        )
