...
from merlintools.vendor.fpd.fpd_file import MerlinBinary, DataBrowser
...
class TestFile(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.tmp_d_multi_ims = tempfile.mkdtemp(
            dir=self.tmp_d, prefix="multiple_single_images-"
        )
        z_fn = os.path.join(
            self.bp, "file_datasets", "zero_values_6bit_multiple_files_16x16.zip"
        )
        with zipfile.ZipFile(z_fn, "r") as z:
            z.extractall(path=self.tmp_d_multi_ims)
            z_multi_ims_fns = z.namelist()
        self.z_mi_dm_fns = [
            x for x in z_multi_ims_fns if x.split(".")[-1].lower() in ["dm3", "dm4"]
        ]
        self.z_mi_bn_fns = [
            x for x in z_multi_ims_fns if x.split(".")[-1].lower() in ["bin", "mib"]
        ]
        self.z_mi_hd_fns = [
            x for x in z_multi_ims_fns if x.split(".")[-1].lower() in ["hdr"]
        ]

        # raw mode
        self.tmp_d_raw = tempfile.mkdtemp(dir=self.tmp_d, prefix="raw-")
        z_fn = os.path.join(
            self.bp, "file_datasets", "medipix_test_binary_data_raw.zip"
        )
        with zipfile.ZipFile(z_fn, "r") as z:
            z.extractall(path=self.tmp_d_raw)
            z_fns = z.namelist()
        self.z_dm_fns_raw = [
            x for x in z_fns if x.split(".")[-1].lower() in ["dm3", "dm4"]
        ]
        self.z_bn_fns_raw = [
            x for x in z_fns if x.split(".")[-1].lower() in ["bin", "mib"]
        ]
        ...
        z_fn = os.path.join(
            self.bp, "file_datasets", "medipix_test_binary_data_images.zip"
        )
        with zipfile.ZipFile(z_fn, "r") as z:
        ...
        z_fn = os.path.join(
            self.bp, "file_datasets", "medipix_test_colour_data_v0p67p0p9.zip"
        )
        with zipfile.ZipFile(z_fn, "r") as z:
            z.extractall(path=self.tmp_d_colour)
            z_fns = z.namelist()
        self.z_dm_fns_colour = [
            x for x in z_fns if x.split(".")[-1].lower() in ["dm3", "dm4"]
        ]
        self.z_bn_fns_colour = [
            x for x in z_fns if x.split(".")[-1].lower() in ["bin", "mib"]
        ]
    ...
    def convert_file(self, i, cpd={}, h5pd={}):
        ...
        mb = MerlinBinary(binfns, hdrfn, dmfns, ds_start_skip=0, row_end_skip=1, **cpd)
    ...
    def test_convert_mi(self):
        ...
        mb = MerlinBinary(binfns, hdrfn, dmfns, ds_start_skip=0, row_end_skip=1)
    ...
    def test_convert_images(self):
        ...
        for i in range(len(self.z_dm_fns_ims)):
            ...
            # plt.matshow(ref_im)
            with h5py.File(
                os.path.join(self.tmp_d_images, tag + ".hdf5"), mode="r"
            ) as h:
    ...
    def test_zForLast_file_browser_kwd_dict(self):
        ...
        d = DataBrowser(
            ...
            fpd_check=True,
            norm="lin",
            nav_im_dict={"cmap": "jet", "vmin": 0, "vmax": 1},
        )
        # d.h5f.file.close()  # for headless plotting
        plt.close("all")
    ...
    def test_scanYXalu(self):
        ...
        scanXalu = (16, "x-axis", "nm")
        self.convert_file(
            i, h5pd={"h5fn": h5fn}, cpd={"scanYalu": scanYalu, "scanXalu": scanXalu}
        )
    ...
    def test_detYXalu(self):
        ...
        detXalu = (256, "x-axis", "nm")
        self.convert_file(
            i, h5pd={"h5fn": h5fn}, cpd={"detYalu": detYalu, "detXalu": detXalu}
        )
    ...
    def test_func(self):
        ...
        self.convert_file(
            i, h5pd={"h5fn": h5fn, "func": my_func, "func_kwargs": {"my_arg": 8}}
        )
    ...
    def test_convert_all(self):
        ...
        mb = MerlinBinary(binfns, hdrfn, dmfns, ds_start_skip=0, row_end_skip=0)
    ...
    def test_tools(self):
        ...
        fpg = h5fn
        isfpd, isknown, vs = fpdf._check_fpd_file(
            fpg, min_version=None, max_version=None
        )
        isfpd, isknown, vs = fpdf._check_fpd_file(
            fpg, min_version=None, max_version=None, silent=True
        )

        tagds, dm_group_paths, dm_filenames = fpdf.hdf5_dm_tags_to_dict(
            fpg, fpd_check=True
        )

        out_dm_filenames = [
            os.path.join(self.tmp_d, t.split(".")[0] + "_extracted")
            for t in dm_filenames
        ]
        fpdf.hdf5_dm_to_bin(fpg, dmfns=out_dm_filenames, fpd_check=True, ow=True)

        fpdf.hdf5_fpd_to_bin(
            fpg,
            fpd_fn=os.path.join(self.tmp_d, "bin_extract.bin"),
            fpd_check=True,
            ow=False,
        )

        fpdf.hdf5_src_to_file(
            fpg,
            src_fn=os.path.join(self.tmp_d, "fpd_file.py"),
            fpd_check=True,
            ow=False,
        )
        ...
        fpd_signals = fpdf.fpd_to_hyperspy(
            fpg,
            fpd_check=True,
            assume_yes=True,
    ...
    def test_v0p67p0p8(self):
        ...
        z_fn = os.path.join(
            self.bp,
            "file_datasets",
            "medipix_test_binary_data_v0p67p0p8_small_16.tar.gz",
        )
    ...
    def test_make_updated_fpd_file(self):
        ...
        new_fn = fpdf.make_updated_fpd_file(
            h5fn,
            os.path.join(self.tmp_d, "mk_updated"),
            func=shift_func,
            func_kwargs=dict(scale=scale),
            ow=True,
            progress_bar=False,
        )
    ...
    def test_zz_update_calibration(self):
        ...
        fpdf.update_calibration(
            fn, scan=[0.1, "p"], detector=[0.2, "q"], colour=[0.3, "r"]
        )
        ...
        fn = os.path.join(self.tmp_d, fn)
        fpdf.update_calibration(
            fn, scan=[0.1, "p"], detector=[0.2, "q"], colour=[None, None]
        )
        t = fpdf.fpd_to_tuple(fn)
