...
class TestFPDP(unittest.TestCase):
    ...
    def setUpClass(self):
        self.radius = 32
        sa = fpd.synthetic_data.shift_array(scan_len=9, shift_min=-2.0, shift_max=2.0)
        self.sa = np.asarray(sa)
        self.im = fpd.synthetic_data.disk_image(
            intensity=128, radius=self.radius, size=256, upscale=8, dtype="uint8"
        )  # float)
        self.data = fpd.synthetic_data.shift_images(
            self.sa, self.im, noise=False, origin="bottom"
        )
        ...
        # nrmse
        self.nrmse_ref_im = fpd.synthetic_data.disk_image(
            intensity=64, radius=32, noise=False
        )
    ...
    def test_com(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=None, nc=None, origin="bottom", progress_bar=False
        )
    ...
    def test_com_threshold_otsu(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=None, nc=None, origin="bottom", thr="otsu", progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_threshold_fixed(self):
        ...
        out = fpdp.center_of_mass(
            self.data, nr=None, nc=None, origin="bottom", thr=thr, progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_threshold_flat(self):
        ...
        out = fpdp.center_of_mass(
            data, nr=None, nc=None, origin="bottom", thr="otsu", progress_bar=False
        )
        ...
        out -= out.mean(1)[
            :, None
        ]  # 1 missing point should not affect mean too badly that test fails
    ...
    def test_com_ck(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=3, nc=3, origin="bottom", progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_ck_rebin(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=3, nc=3, rebin=2, origin="bottom", progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_ck_non_parallel(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=3, nc=3, parallel=False, origin="bottom", progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_ck_parallel_ncores(self):
        print(self.id())
        out = fpdp.center_of_mass(
            self.data, nr=3, nc=3, ncores=1, origin="bottom", progress_bar=False
        )
        out -= out.mean((1, 2))[:, None, None]
        self.assertTrue(close_enough(self.sa, out, self.rtol, self.atol))
    ...
    def test_com_aperture(self):
        print(self.id())
        ap = fpdp.virtual_apertures(
            self.data.shape[-2:],
            cyx=(128,) * 2,
            rio=(0, int(self.radius * 1.5)),
            sigma=0,
            aaf=1,
        )
        out = fpdp.center_of_mass(
    ...
    def test_sum_diff_dyx(self):
        ...
        sa = np.asarray(
            fpd.synthetic_data.shift_array(scan_len=3, shift_min=-16.0, shift_max=16.0)
        )
        sa[0] += 2
        sa[1] += 4
        ...
        sum_dif_aligned_zero_filled_pix = fpdp.sum_dif(
            data, None, None, dyx=-sa, dyx_mode="pixel", fill_value=0
        ) / (sa.size / 2)
        sum_dif_aligned_zero_filled_linear = fpdp.sum_dif(
            data, None, None, dyx=-sa, dyx_mode="linear", fill_value=0
        ) / (sa.size / 2)
        sum_dif_aligned_zero_filled_fourier = fpdp.sum_dif(
            data, None, None, dyx=-sa, dyx_mode="fourier", fill_value=0
        ) / (sa.size / 2)
    ...
    def test_synthetic_aperture(self):
        ...
        try:
            aps = fpdp.synthetic_aperture(
                (64,) * 2, (32,) * 2, (0, 16), 0.5, ds_method="banana"
            )
        except:
            pass
        else:
        ...
        aps = fpdp.synthetic_aperture(
            (64,) * 2, (32,) * 2, (0, 16), 0.5, ds_method="interp"
        )
        aps = fpdp.synthetic_aperture(
            (64,) * 2, (32,) * 2, (0, 16), 0.5, ds_method="rebin"
        )

    def test_synthetic_images(self):
        print(self.id())
        aps = fpdp.synthetic_aperture(
            self.data.shape[-2:], (128,) * 2, np.linspace(16, 192, 4)
        )
        synth_ims = fpdp.synthetic_images(
            self.data, nr=3, nc=3, apertures=aps, progress_bar=False
        )
    ...
    def test_synthetic_images_rebin(self):
        ...
        synth_ims = fpdp.synthetic_images(
            self.data, nr=3, nc=3, apertures=aps, rebin=2, progress_bar=False
        )

        synth_ims = fpdp.synthetic_images(
            self.data, nr=3, nc=3, apertures=aps, rebin=3, progress_bar=False
        )
    ...
    def test_virtual_apertures_images_single_multi(self):
        ...
        virt_ims_multi = fpdp.virtual_images(
            self.data, nr=3, nc=3, apertures=aps_multi, progress_bar=False
        )
        virt_ims_single = fpdp.virtual_images(
            self.data, nr=3, nc=3, apertures=aps_multi[0], progress_bar=False
        )
    ...
    def test_virtual_images_dyx(self):
        ...
        intensity = 128
        im = fpd.synthetic_data.disk_image(
            intensity=intensity,
            radius=self.radius,
            size=256,
            upscale=8,
        ...
        aps = fpdp.virtual_apertures(
            ...
            cyx,
            np.array(
                [[0, self.radius - pad], [self.radius + pad, self.radius + 8 + 20]]
            ),
            sigma=0,
        )
        ...
        ## no shifts
        data0 = fpd.synthetic_data.shift_images(
            np.zeros((2, scan_len, scan_len)), im, noise=False, origin="top"
        )
        virt_ims0 = fpdp.virtual_images(
            data0, nr=None, nc=None, apertures=aps, progress_bar=False, debug=False
        )
        ...
        for sa_bias in [-10, 0, 10]:
            ...
            sa = np.asarray(
                fpd.synthetic_data.shift_array(
                    scan_len=scan_len, shift_min=-8.1, shift_max=8.1
                )
            )
            sa -= sa_bias
            ...
            for dyx_mode in ["linear", "pixel", "fourier"]:
                with Timer(dyx_mode):
                    virt_ims_aligned = fpdp.virtual_images(
                        data,
                        nr=None,
                        nc=None,
                        apertures=aps,
                    ...
                    assert np.allclose(
                        virt_ims_aligned[1], virt_ims0[1], rtol=rtol, atol=atol
                    )
                    assert np.allclose(
                        virt_ims_aligned[0], virt_ims0[0], rtol=rtol, atol=atol
                    )
            plt.close("all")
        ...
        for sa_bias in [-10, 0, 10]:
            ...
            sa = np.asarray(
                fpd.synthetic_data.shift_array(
                    scan_len=scan_len, shift_min=-80.1, shift_max=80.1
                )
            )
            sa -= sa_bias
            data = fpd.synthetic_data.shift_images(sa, im, noise=False, origin="top")
    ...
    def test_synthetic_images_virtual_images_speed(self):
        ...
        with Timer("synthetic images"):
            synth_ims = fpdp.synthetic_images(
                self.data, nr=None, nc=None, apertures=aps, progress_bar=False
            )
    ...
    def test_sum_im_nrnc_are_chunks(self):
        print(self.id())
        sum_im = fpdp.sum_im(
            self.data, nr=3, nc=3, nrnc_are_chunks=True, progress_bar=False
        )
    ...
    def test_radial_profile(self):
        ...
        mask = np.ones(im_shape, dtype=int)
    ...
    def test_radial_profiles(self):
        ...
        intensity = 128
        im = fpd.synthetic_data.disk_image(
            intensity=intensity, radius=16, size=128, upscale=8, dtype="uint8", sigma=0
        )

        # data with shifts
        sa = np.asarray(
            fpd.synthetic_data.shift_array(scan_len=5, shift_min=-12, shift_max=12)
        )
        data = fpd.synthetic_data.shift_images(sa, im, noise=False, origin="top")
        ...
        for r_lim_mode in ["corner_max", "corner_min", "edge_max", "edge_min"]:
            ...
            r, rm = fpdp.radial_profiles(
                data,
                16,
                16,
                cyx,
                r_lim_mode=r_lim_mode,
                progress_bar=False,
                print_stats=False,
            )
    ...
    def test_condition_nrnc_if_chunked(self):
        print(self.id())
        rtn = fpdp._condition_nrnc_if_chunked(
            data=None, nr=None, nc=None, print_enabled=True
        )
    ...
    def test_matching(self):
        ...
        aperture = fpdp.virtual_apertures(
            images.shape[-2:], cyx=(128,) * 2, rio=(0, 48), sigma=0, aaf=1
        )

        matching = fpdp.find_matching_images(
            images, aperture, plot=True, progress_bar=False
        )
        ...
        # test no plotting
        matching = fpdp.find_matching_images(
            images, aperture, plot=False, progress_bar=False
        )

        # test no aperture
        matching = fpdp.find_matching_images(
            images, aperture=None, plot=False, progress_bar=False
        )
    ...
    def test_find_circ_centre(self):
        ...
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(2, int(image.shape[0] / 2.0), 1), plot=False
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=True
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=False, pct=90
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=False, mask=mask
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=False, mask=mask, pct=90
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=False, mask=mask, pct=90, spf=2
        )
        plt.close("all")

        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=True, subpix=False
        )
        cyx, cr = fpdp.find_circ_centre(
            image, sigma=6, rmms=(r - 4, r + 4, 1), plot=False, fit_hw=4
        )

        # multiple discs
        from fpd.synthetic_data import shift_im

        im2 = fpd.synthetic_data.disk_image(
            intensity=32, radius=24, sigma=2, size=256, noise=False
        )
        shifted_im = shift_im(im2, dyx=(64, 64), noise=False)
        im_multidiscs = image + shifted_im
        # plt.matshow(im_multidiscs)
        cyx, cr = fpdp.find_circ_centre(
            im_multidiscs, sigma=6, rmms=(20, 36, 1), plot=True, max_n=2
        )
        # print(cyx, cr)
        cyx, cr = fpdp.find_circ_centre(
            im_multidiscs, sigma=6, rmms=(20, 36, 1), plot=False, max_n=2
        )
        # print(cyx, cr)
        plt.close("all")
    ...
    def test_map_image_function(self):
        ...
        out = fpdp.map_image_function(
            self.data, nr=None, nc=None, func=func, progress_bar=False
        )
        assert out.ndim == 3
    ...
    def test_map_image_function_nonuniform(self):
        print(self.id())
        out = fpdp.map_image_function(
            self.data, nr=None, nc=None, func=nonuniform_f, progress_bar=False
        )
        assert out.ndim == 2
    ...
    def test_map_image_function_ck(self):
        ...
        func = sp.ndimage.center_of_mass
        out = fpdp.map_image_function(
            self.data, nr=3, nc=3, func=func, progress_bar=False
        )
    ...
    def test_map_image_function_ck_nrnc_are_chunks(self):
        ...
        func = sp.ndimage.center_of_mass
        out = fpdp.map_image_function(
            self.data, nr=3, nc=3, func=func, nrnc_are_chunks=True, progress_bar=False
        )
    ...
    def test_map_image_function_ck_rebin(self):
        ...
        func = sp.ndimage.center_of_mass
        out = fpdp.map_image_function(
            self.data, nr=3, nc=3, rebin=2, func=func, progress_bar=False
        )
    ...
    def test_map_image_function_ck_non_parallel(self):
        ...
        func = sp.ndimage.center_of_mass
        out = fpdp.map_image_function(
            self.data, nr=3, nc=3, parallel=False, func=func, progress_bar=False
        )
    ...
    def test_map_image_function_ck_parallel_ncores(self):
        ...
        func = sp.ndimage.center_of_mass
        out = fpdp.map_image_function(
            self.data, nr=3, nc=3, ncores=1, func=func, progress_bar=False
        )
    ...
    def test_VirtualAnnularImages(self):
        print(self.id())
        V = fpdp.VirtualAnnularImages(
            self.data, nr=3, nc=3, cyx=(128,) * 2, progress_bar=False
        )
        ...
        V.plot(
            ...
            r2=30,
            norm="log",
            nav_im=np.ones(self.data.shape[-2:]),
            alpha=0.2,
            cmap="gray",
            pct=1,
        )
    ...
    def test_VirtualAnnularImages_acc(self):
        ...
        for spf in [1, 2]:
            v = fpdp.VirtualAnnularImages(
                ims, 16, 16, cyx=(16, 16), spf=spf, progress_bar=False
            )
    ...
    def test_VirtualAnnularImages_dyx(self):
        ...
        sa = np.asarray(
            fpd.synthetic_data.shift_array(scan_len=9, shift_min=-16.0, shift_max=16.0)
        )
        data = fpd.synthetic_data.shift_images(sa, im, noise=False, origin="top")

        pad = 2
        V0 = fpdp.VirtualAnnularImages(
            data, nr=3, nc=3, cyx=(128,) * 2, dyx=None, progress_bar=False
        )
    ...
    def test_make_ref_im(self):
        ...
        edge_sigma = fpdp.disc_edge_properties(
            image, sigma=2, cyx=cyx, r=cr, plot=False
        ).sigma_wt_avg
        aperture = fpdp.virtual_apertures(
            image.shape[-2:], cyx=cyx, rio=(0, cr + 32), sigma=0, aaf=1
        )
        ...
        t3 = fpdp.make_ref_im(
            image, edge_sigma, aperture, bin_opening=4, bin_closing=4, plot=False
        )
        t4 = fpdp.make_ref_im(
            image,
            edge_sigma,
    ...
    def test_shift_images(self):
        ...
        with Timer("sub-pixel mode with sub-pixel shifts (linear)"):
            sa = np.asarray(
                fpd.synthetic_data.shift_array(
                    scan_len=9, shift_min=-2.0, shift_max=2.0
                )
            )
            data = fpd.synthetic_data.shift_images(
                sa, self.im, noise=False, origin="top"
            )
            ...
            self.assertTrue(
                close_enough(
                    out - self.cyx[..., None, None], dyx_ref, self.rtol, self.atol
                )
            )
        ...
        with Timer("sub-pixel mode with sub-pixel shifts (fourier)"):
            ...
            unshifted_data = fpdp._shift_images(
                data.copy(), dyx=-sa, dyx_mode="fourier"
            )
            out = fpdp.center_of_mass(
                unshifted_data,
                nr=None,
        ...
        with Timer("sub-pixel mode with pixel shifts"):
            sa = np.asarray(
                fpd.synthetic_data.shift_array(
                    scan_len=9, shift_min=-4.0, shift_max=4.0
                )
            )
            data = fpd.synthetic_data.shift_images(
                sa, self.im, noise=False, origin="top"
